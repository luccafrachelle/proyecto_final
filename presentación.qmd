...
* $v_{ij}$: Probabilidad de conexión entre los puntos $X_i$ y $X_j$ en **alta dimensión** (grafo de vecinos).
* $w_{ij}$: Probabilidad de conexión entre los puntos $y_i$ y $y_j$ en **baja dimensión** (embedding).




## Descenso de Gradiente Estocástico{.smaller}

La optimización de la incrustación $Y$ se realiza mediante **Descenso de Gradiente Estocástico (SGD)**. El proceso sigue un bucle iterativo que ajusta las posiciones de los puntos simulando un sistema de fuerzas. El algoritmo, en esencia, hace lo siguiente:

Por cada **eje** $(i, j)$ con peso $v_{ij}$ que conecta a dos vecinos en el grafo de alta dimensión:

1.  **Muestreo de Atracción (Ejes Positivos):** Se realiza un "sorteo". Con una probabilidad igual al peso del eje ($v_{ij}$), se decide si se procesará este eje en la iteración actual. Los ejes más fuertes (puntos más similares) se eligen con más frecuencia.

2.  **Si el eje es seleccionado, se aplican dos fuerzas secuencialmente:**
    *   **a) Fuerza de Atracción:** Se aplica una fuerza para acercar las representaciones $y_i$ y $y_j$ en el mapa. La actualización mueve $y_i$ en la dirección que maximiza su similitud ($w_{ij}$):
        $$y_i \leftarrow y_i + \alpha \cdot \nabla_{y_i}(\log(w_{ij}))$$

    *   **b) Fuerza de Repulsión (Muestreo Negativo):** Inmediatamente después, para evitar que los puntos se aglomeren, se aplica una fuerza contraria. El algoritmo elige uno o más puntos $k$ al azar del dataset (estos son los "ejemplos negativos") y aplica una fuerza que aleja a $y_i$ de cada $y_k$:
        $$y_i \leftarrow y_i + \alpha \cdot \nabla_{y_i}(\log(1 - w_{ik}))$$

Este ciclo de **muestreo-atracción-repulsión** se repite a lo largo de muchas épocas. Con cada época, la tasa de aprendizaje $\alpha$ disminuye, permitiendo que el sistema se asiente en un equilibrio estable. El resultado final es un mapa de baja dimensión donde la estructura topológica de los datos originales queda preservada.
...
# Comparativa t-SNE vs UMAP {.smaller}
Ambas t-SNE y UMAP son herramientas poderosas para la reducción de dimensionalidad no lineal y la visualización.

| Característica                 | t-SNE                                                              | UMAP                                                                        |
|:-------------------------------|:-------------------------------------------------------------------|:----------------------------------------------------------------------------|
| **Preservación de Estructura** | Excelente para estructura local; distorsiona la global.            | Excelente para estructura local y global; distancias entre clusters significativas. |
| **Velocidad y Escalabilidad**  | Generalmente lento ($O(N^2)$); puede ser $O(N \log N)$ con optimizaciones (ej. Barnes-Hut); no ideal para datasets grandes.     | Significativamente más rápido. Su complejidad es $O(N \log N)$ para la construcción del grafo y luego $O(N)$ para la optimización. Puede manejar datasets de millones de puntos. |
| **Reproductibilidad**          | Muy sensible a parámetros; visualizaciones pueden variar.          | Más robusto y consistente entre ejecuciones.                                |
| **Forma de los Clusters**      | Tiende a producir clusters densos y circulares.                    | Puede producir clusters con formas diversas y mejor separación visual.      |
| **Nuevos Datos**               | No aprende función de mapeo; no proyecta nuevos datos sin reentrenar. | Aprende función de mapeo; puede transformar eficientemente nuevos datos.   |
...
def plot_optimization_process(X, y, n_steps=6):
    ...
    for i, iter in enumerate(iterations_to_plot):
        tsne = TSNE(n_components=2, 
                   ...
                   random_state=42,
                   perplexity=30,
                   n_iter=iter)#max_iter según la versión de python
...
def plot_umap_optimization_process(X, y, n_steps=6):
    ...
    with warnings.catch_warnings():
        ...
        for i, n_epochs in enumerate(epochs_to_plot):
            ...
            ax.set_title(f'Iteración: {n_epochs}', fontsize=12, fontweight='bold')
...
## Predicción de dígitos con UMAP como preprocesamiento
...
df_metrics_simple_per_digit = pd.DataFrame(report_simple_dict).transpose().drop(
    ['f1-score', 'macro avg', 'weighted avg'], errors='ignore'
)
